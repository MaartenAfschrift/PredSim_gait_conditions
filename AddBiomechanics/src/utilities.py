...
def detect_events_running(time, F, ankle_rx, ankle_lx, treshold = 50, dtOffPlate = None):
    # detect gait events based on the assumption that subjects is running in the x-direction
    # this algorithm only works when there is a flight phase
    # optional settings dtOffPlate gives the minimal time of the flight phase.
    # default script to detect gait events
    [ths, tto, hs, to] = detect_heelstrike_toeoff(time, F, treshold, dtOffPlate = dtOffPlate)

    # assign heelstrikes to left or right foot based on x coordinate at heelstrike
    [ct_hsl, ct_hsr, ct_tol, ct_tor] = 0, 0, 0, 0
    [ths_l, ths_r, tto_l, tto_r] = (np.empty([len(hs), 1]),
                                    np.empty([len(hs), 1]),
                                    np.empty([len(hs), 1]),
                                    np.empty([len(hs), 1]))
    for i in range(0,len(ths)):
        if (ankle_rx[hs[i]] > ankle_lx[hs[i]]):
            ths_r[ct_hsr] = ths[i]
            ct_hsr = ct_hsr + 1
        else:
            ths_l[ct_hsl] = ths[i]
    ...
    # similar for toe-off, but we search for foot with the minimal x-coordinate
    for i in range(0, len(tto)):
        if (ankle_rx[to[i]] < ankle_lx[to[i]]):
            tto_r[ct_tor] = tto[i]
            ct_tor = ct_tor + 1
        else:
            tto_l[ct_tol] = tto[i]
    ...
    # delete pre-allocated frames
    ths_l = np.delete(ths_l, np.s_[ct_hsl:len(hs)])
    ths_r = np.delete(ths_r, np.s_[ct_hsr:len(hs)])
    tto_l = np.delete(tto_l, np.s_[ct_tol:len(hs)])
    tto_r = np.delete(tto_r, np.s_[ct_tor:len(hs)])

    return ths_l, ths_r, tto_l, tto_r




# norm cycle to N datapoints
def norm_cycle(time, EventVector, data, treshold_dtStride_max=99999, treshold_dtStride_min=0.1, bool_print = True, n_points_int = 100):
    ...
    for i in range(n_step):
        ...
        if (dtStride < treshold_dtStride_max and dtStride > treshold_dtStride_min):
