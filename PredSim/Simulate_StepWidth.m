...
%% Loop over imposed stride frequency
...
StepWidthV = 0.09:0.03:0.5;

for i=1:length(StepWidthV)
    % adapt stride width
    SWName = num2str(round(StepWidthV(i)*100));

    % constrain the stepwidth
    S.bounds.calcn_dist.lower = StepWidthV(i);

    % select subject
    S.subject.save_folder  = fullfile(saveFolderMain,'PredSimResults',['SW_' SWName]);
