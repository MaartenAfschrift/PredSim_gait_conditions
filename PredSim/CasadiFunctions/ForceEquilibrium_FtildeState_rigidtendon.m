function [err, FT, Fce, Fpass, Fiso, vMmax, massM] = ...
    ForceEquilibrium_FtildeState_rigidtendon(a,fse,lMT,vMT,FMo_in,lMo_in,...
    lTs_in,alphao_in,vMmax_in,Fvparam,Fpparam,Faparam,tension,...
    MuscMoAsmp,d,stiffness_shift,stiffness_scale,strength,S)
% --------------------------------------------------------------------------
...
if ~S.subject.IgnoreFLVmuscle
    ...
    % ignore muscle force/length/velocity properties
    FMltilde = 1;
    FMvtilde = 1;
    vT = 0;
    if(MuscMoAsmp == 0) % b = cst
...
if ~ S.subject.IgnorePasFLmuscle
    e0 = 0.6;
    kpe = 4;
