function [f_forceEquilibrium_FtildeState_rigidtendon,f_FiberLength_TendonForce_rigidtendon,...
    f_FiberVelocity_TendonForce_rigidtendon,f_lT_vT_rigid] = createCasadi_ContractDynam_rigidTendon(S,model_info)
% --------------------------------------------------------------------------
...
for m = 1:N_muscles
    [Hilldiff(m),FT(m),Fce(m),Fpass(m),Fiso(m),vMmax(m),massM(m)] = ...
        ForceEquilibrium_FtildeState_rigidtendon(a(m),FTtilde(m),...
        lMT(m),vMT(m),model_info.muscle_info.parameters(m).FMo,...
        ...
        Fvparam,Fpparam,Faparam,tension_SX(m),S.misc.constant_pennation_angle,...
...
f_forceEquilibrium_FtildeState_rigidtendon = ...
    Function('f_forceEquilibrium_FtildeState_rigidtendon',{a,FTtilde,...
    lMT,vMT,tension_SX},{Hilldiff,FT,Fce,Fpass,Fiso,vMmax,massM},...
    {'a','FTtilde','lMT','vMT','tension_SX'},...
...
lT      = SX(N_muscles,1);
for m = 1:N_muscles
    [lM(m),lMtilde(m),lT(m)] = FiberLength_TendonForce_rigidtendon(FTtilde(m),...
        model_info.muscle_info.parameters(m).lMo,model_info.muscle_info.parameters(m).lTs,...
        model_info.muscle_info.parameters(m).alphao,lMT(m),...
        S.misc.constant_pennation_angle);
end
f_FiberLength_TendonForce_rigidtendon = Function(...
    'f_FiberLength_Ftilde_rigidtendon',{FTtilde,lMT},{lM,lMtilde},...
...
vT      = SX(N_muscles,1);
% FTtile,lMo_in,lTs_in,alphao_in,vMmax_in,lMT,vMT,MuscMoAsmp
for m = 1:N_muscles
    [vM(m),vMtilde(m),vT(m)] = FiberVelocity_TendonForce_rigidtendon(FTtilde(m),...
        model_info.muscle_info.parameters(m).lMo,model_info.muscle_info.parameters(m).lTs,...
        model_info.muscle_info.parameters(m).alphao,model_info.muscle_info.parameters(m).vMmax,lMT(m),...
        vMT(m),S.misc.constant_pennation_angle);
end
f_FiberVelocity_TendonForce_rigidtendon = Function(...
    'f_FiberVelocity_Ftilde_rigidtendon',{FTtilde,lMT,vMT},...
    {vM,vMtilde},{'FTtilde','lMT','vMT'},{'vM','vMtilde'});

f_lT_vT_rigid = Function('f_lT_vT_rigid',{FTtilde,lMT,vMT},...
    {lT,vT},{'FTtilde','lMT','vMT'},{'lT','vT'});
end