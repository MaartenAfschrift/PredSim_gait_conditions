...
for i = 1:length(SlopesV)
    ...
    SubjName = ['Fall22_slope' slope_str '.osim'];
    OutPathModels = fullfile(pathRepo,'Subjects',['Fall22_slope' slope_str]);
    if ~isfolder(OutPathModels)
        mkdir(OutPathModels);
    end
    OutPathModel = fullfile(OutPathModels,SubjName);
    ...
    osim2cpp(S.Cpp2Dll,OutPathModels,OutPathModel)

    % convert cpp
    outputFilename = ['Fall22_slope' slope_str];
    verbose = 1;
    cpp2dll(OutPathModels,outputFilename,[],Compiler,S.Cpp2Dll.PathCpp2Dll_Exe,verbose);
end
