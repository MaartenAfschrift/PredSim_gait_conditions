...
for j=1:d
    ...
    if ~S.subject.RigidTendon
        ...
        [Hilldiffj,FTj,Fcej,Fpassj,Fisoj] = ...
            f_casadi.forceEquilibrium_FtildeState_rigidtendon(akj(:,j+1),...
            FTtildekj_nsc(:,j+1),lMTj,vMTj,tensions);
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ~S.subject.RigidTendon
        ...
        [~,lMtildej] = f_casadi.FiberLength_TendonForce_rigidtendon(...
            FTtildekj_nsc(:,j+1),lMTj);
        % Get muscle fiber velocities
        [vMj,~] = f_casadi.FiberVelocity_TendonForce_rigidtendon(...
            FTtildekj_nsc(:,j+1),lMTj,vMTj);
    end
    ...
    if isfield(S,'objective') && isfield(S.objective,'MinMechWork') && S.objective.MinMechWork
        % mechanical work muscle fibers
        Pmuscle = -Fcej.*vMj;
        Pmusclesq = sum(Pmuscle.^2);

        % add mechanical work torque actuators
        P_T_sq = 0;
        for i=1:nq.all
            if nq.torqAct > 0 && ismember(i,model_info.ExtFunIO.jointi.torqueActuated)
                ...
                qd_i = qdotinj(i);
                P_T_sq = P_T_sq+ (T_act_i*qd_i).^2;
            end    
        end
        Pmusclesq = Pmusclesq + P_T_sq;

        % 
         J = J + ...
            W.E          * B(j+1) *Pmusclesq/model_info.mass*h + ...
...
% 
    ...
    for i=1:nq.all
        ...
        Ti = 0;
        if strcmp(model_info.ExtFunIO.coord_names.all{i},'pelvis_ty') && S.PelvisFyResBool
            Ti = S.PelvisFyRes;
        end
        if strcmp(model_info.ExtFunIO.coord_names.all{i},'pelvis_tx') && S.PelvisFxResBool
            Ti = S.PelvisFxRes;
        end
...
for k=1:N
    for j=1:d
        ...
        if ~S.subject.RigidTendon
            ...
            [~,~,Fce_opt_all,Fpass_opt_all,Fiso_opt_all] = ...
                f_casadi.forceEquilibrium_FtildeState_rigidtendon(...
                a_col_opt_unsc(count,:)',FTtilde_col_opt_unsc(count,:)',...
                full(lMTkj_opt_all),full(vMTkj_opt_all),tensions);
            % muscle-tendon kinematics
            [~,lMtilde_opt_all] = f_casadi.FiberLength_TendonForce_rigidtendon(...
                FTtilde_col_opt_unsc(count,:)',full(lMTkj_opt_all));
            [vM_opt_all,~] = f_casadi.FiberVelocity_TendonForce_rigidtendon(...
                FTtilde_col_opt_unsc(count,:)',full(lMTkj_opt_all),...
                full(vMTkj_opt_all));
        end
