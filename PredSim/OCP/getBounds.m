...
for i = 1:NCoord
    ...
    if strcmp(coordinate,'pelvis_tx') && S.BackWardWalk
        bounds.Qs.upper(coord_idx) = 4;
        bounds.Qs.lower(coord_idx) = -4;
        % Qdots
        bounds.Qdots.upper(coord_idx) = 3;
        bounds.Qdots.lower(coord_idx) = -4;
        % Qdotdots
        bounds.Qdotdots.upper(coord_idx) = 20;
        bounds.Qdotdots.lower(coord_idx) = -20;
    else        
...
bounds.Qs.upper(model_info.ExtFunIO.symQs.QsInvA) =...
    max(bounds.Qs.upper(model_info.ExtFunIO.symQs.QsInvA),...
    bounds.Qs.upper(model_info.ExtFunIO.symQs.QsInvB));
bounds.Qs.lower(model_info.ExtFunIO.symQs.QsInvA) =...
    min(bounds.Qs.lower(model_info.ExtFunIO.symQs.QsInvA),...
    bounds.Qs.lower(model_info.ExtFunIO.symQs.QsInvB));

bounds.Qdots.upper(model_info.ExtFunIO.symQs.QdotsInvA) =...
    max(bounds.Qdots.upper(model_info.ExtFunIO.symQs.QdotsInvA),...
    bounds.Qdots.upper(model_info.ExtFunIO.symQs.QdotsInvB));
bounds.Qdots.lower(model_info.ExtFunIO.symQs.QdotsInvA) =...
    min(bounds.Qdots.lower(model_info.ExtFunIO.symQs.QdotsInvA),...
    bounds.Qdots.lower(model_info.ExtFunIO.symQs.QdotsInvB));

bounds.Qdotdots.upper(model_info.ExtFunIO.symQs.QdotsInvA) =...
    max(bounds.Qdotdots.upper(model_info.ExtFunIO.symQs.QdotsInvA),...
    bounds.Qdotdots.upper(model_info.ExtFunIO.symQs.QdotsInvB));
bounds.Qdotdots.lower(model_info.ExtFunIO.symQs.QdotsInvA) =...
    min(bounds.Qdotdots.lower(model_info.ExtFunIO.symQs.QdotsInvA), ...
    bounds.Qdotdots.lower(model_info.ExtFunIO.symQs.QdotsInvB));
...
if S.BackWardWalk
    bounds.Qs.upper(model_info.ExtFunIO.jointi.floating_base(4)) = 4;  
    bounds.Qs.lower(model_info.ExtFunIO.jointi.floating_base(4)) = -8;
end
% Pelvis_ty
...
if S.subject.v_pelvis_x_trgt < -1.33
    ...
    bounds.Qs.lower(idx_shoulder_flex) = -50*pi/180;
    % Pelvis tx
    bounds.Qdots.lower(model_info.ExtFunIO.jointi.floating_base(4)) = -8;
end
...
if strcmp(S.misc.gaitmotion_type,'HalfGaitCycle')
    ...
    bounds.tf.lower = S.bounds.t_final.lower/2;
else
