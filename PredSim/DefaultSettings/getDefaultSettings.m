...
% rigid tendon
if ~isfield(S.subject,'RigidTendon')
    S.subject.RigidTendon = false;
end

% ignore force length and force velocity properties
if ~isfield(S.subject,'IgnoreFLVmuscle')
    S.subject.IgnoreFLVmuscle = false;
end

if ~isfield(S.subject,'IgnorePasFLmuscle')
    S.subject.IgnorePasFLmuscle = false;
end
...
% pelvis Fy residuals
if ~isfield(S,'PelvisFyResBool')
    S.PelvisFyResBool = false;
end
if ~isfield(S,'PelvisFyRes')
    S.PelvisFyRes = 0;
end

% pelvis Fx residuals
if ~isfield(S,'PelvisFxResBool')
    S.PelvisFxResBool = false;
end
if ~isfield(S,'PelvisFxRes')
    S.PelvisFxRes = 0;
end

if ~isfield(S,'BackWardWalk')
    S.BackWardWalk = false;
end
